...
aws --region $AWS_DEFAULT_REGION s3api put-object \
    --bucket $AWS_S3_BUCKET --key "${AWS_S3_PREFIX}-${PARAMETERS_FILE}-${EPOCHS}.params" \
    --body "${PARAMETERS_FILE}-${EPOCHS}.params" >/dev/null

rc=$?
if [ $rc -ne 0 ] ;then
  echo "[Error] Sending job results (params) went wrong..." 1>&2
  echo "AWS_S3_BUCKET: "${AWS_S3_BUCKET} 1>&2
...
aws --region $AWS_DEFAULT_REGION s3api put-object \
    --bucket $AWS_S3_BUCKET --key "${AWS_S3_PREFIX}-${PARAMETERS_FILE}-symbol.json" \
    --body "${PARAMETERS_FILE}-symbol.json" >/dev/null

rc=$?
if [ $rc -ne 0 ] ;then
  echo "[Error] Sending job results (symbol) went wrong..." 1>&2
  echo "AWS_S3_BUCKET: "${AWS_S3_BUCKET} 1>&2
